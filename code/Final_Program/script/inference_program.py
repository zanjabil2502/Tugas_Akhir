...
config_path = 'result/threshold_detection/80_persen/segmentation/method_02/data_07/42/panns/config.json'
...
checkpoint_path = 'result/threshold_detection/80_persen/segmentation/method_02/data_07/42/panns/best_checkpoint.pt'
...
def predict(filename):
    ...
    if value < 80:
        print('Your Cough not detection')
    elif value >= 80:
        ...
        if max_time < 1:
            ...
            pad_wav = []
            for value in segm_wav:
                pad = lb.util.pad_center(value,max_time*fs)
                pad_wav.append(pad)
        ...
        y_pred_tag = torch.round(output_mean)
        ...
        print(category[preds[0]])

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-f', '--filename', type=str, required=True,
                        help="filename audio example: ../compare/dist/wav")
